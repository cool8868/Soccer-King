...
/// Represents the passive type skill effector
class SkillEffector 
{
public:
...
public:
    ...
    void Effect(IPlayer* player, IPassiveRawSkill* skill) 
    {
        vector<int>& formation = skill->Formation();

        if (formation.size() == 0) 
        {
            throw SkillErrorException("Skill Config error. The passive skill has no formation config.");
        }
