...
#include "../Cache/ConfigCache/OpenerSkillConfigCache.h"
#include "../Entity/OpenerRawSkill.h"
#include "../Entity/Parts/ManagerChangesSkillPart.h"

#include "../../common/Utility.h"

OpenerSkillBuilder::OpenerSkillBuilder()
    : SkillBuilder("OpenerSkillBuilderConfig") 
{
}

IRawSkill* OpenerSkillBuilder::BuildSkill(string skillId) 
{
    try 
    {
        ...
        {
            throw ObjectNotInitializedException("builder not initialized. builder name:" + TypeId(OpenerSkillBuilder));
        }

        xml_node& skillElement = singleton_default<OpenerSkillConfigCache>::instance().GetSkill(skillId);
        ...
        if (skillName == NULL) 
        {
            format builder("Can't find the skill's name from the config file. Skill Id: %");
        ...
        vector<int>& formationIds = Common::TransToValueVector<int>(formation.value(), ",");

        OpenerRawSkill* skill = new OpenerRawSkill(skillId, skillName.value(), lexical_cast<int>(last.value()), formationIds);

        foreach (MapString_ISkillPartBuilder::value_type& builder, m_Builders) 
        {
            xml_node& partElement = singleton_default<OpenerSkillConfigCache>::instance().GetSkillPart(builder.first, skillElement);
            ...
            // 设置OpenerRawSkill对象到ManagerChangesSkillPart中
            AttatchToManagerChangesSkillPart(partElement, skill, skillPart);

            // 设置各个Part到OpenerRawSkill对象中
    ...
    {
        throw SkillErrorException("Invokes OpenerSkillBuilder class Build method throw exceptions. Skill Id:" + skillId, ex);
    }
}

void OpenerSkillBuilder::AttatchToManagerChangesSkillPart(xml_node& node, IManagerSkill* skill, ISkillPart* skillPart)
{
