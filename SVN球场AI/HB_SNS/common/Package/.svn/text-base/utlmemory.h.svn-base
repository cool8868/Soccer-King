...
//
// $Header: /cvs/myengine/src_main/public/valve/utlmemory.h,v 1.1 2008/02/29 00:05:49 easap\albertyang Exp $
// $NoKeywords: $
//
...
template< class T >
CUtlMemory<T>::CUtlMemory( CUtlMemory<T>& m ) : m_pMemory(0), 
m_nAllocationCount( m.m_nAllocationCount ), m_nGrowSize( m.m_nGrowSize )
{
	if (!IsExternallyAllocated() && m_nAllocationCount > 0)
	{
		m_pMemory = (T*)malloc( m_nAllocationCount * sizeof(T) );

		Q_memcpy(m_pMemory, m.m_pMemory, m_nAllocationCount * sizeof(T));
	}
}
